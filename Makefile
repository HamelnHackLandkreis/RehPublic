...
backend-run:
	cd backend && set -a && . .env && set +a && uv run python download_deepfaune_model.py
	cd backend && set -a && . .env && set +a && PYTHONPATH=src uv run uvicorn api.main:app --reload --port 8000

backend-run-workers:
	cd backend && set -a && . .env && set +a && uv run python download_deepfaune_model.py
	cd backend && set -a && . .env && set +a && PYTHONPATH=src uv run uvicorn api.main:app --port 8000 --workers 4

backend-run-celery:
	cd backend && set -a && . .env && set +a && PYTHONPATH=src uv run celery -A src.celery_app worker --loglevel=info --concurrency=1
...
backend-sync-images:
	cd backend && set -a && . .env && set +a && PYTHONPATH=src uv run python trigger_image_sync.py
...
run: backend-sync frontend-prep redis-start
	...
	@sleep 2
	@cd backend && set -a && . .env && set +a && uv run python download_deepfaune_model.py && PYTHONPATH=src uv run uvicorn api.main:app --reload --port 8000 & \
	cd backend && set -a && . .env && set +a && PYTHONPATH=src uv run celery -A src.celery_app worker --loglevel=info --concurrency=1 & \
	cd frontend && npm run dev
