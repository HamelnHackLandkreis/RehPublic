<template>
  <div
    class="flex flex-grow px-4 sm:px-6 lg:px-8 py-6 overflow-x-hidden w-full overflow-y-auto"
  >
    <!-- Loading State -->
    <div
      v-if="loading"
    ...
    <div
      v-else-if="error"
      class="flex flex-col items-center justify-center flex-1 gap-4 text-red-600"
    >
      <p>{{ error }}</p>
      <button
        @click="fetchCameras"
    ...
    <div v-else class="w-full max-w-7xl mx-auto">
      ...
      >
        <div>
          ...
          <p class="text-base text-white/90 drop-shadow">
            Choose a camera to view its details, statistics, and images
          </p>
        </div>
        <button
      ...
      <div
        ...
        class="text-center py-12 text-white/80 text-base"
      >
      ...
      <div
        v-else
        class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-5 w-full"
      >
        <button
        ...
        >
          ...
          >
            <img
              v-if="
                camera.images && camera.images.length > 0 && camera.images[0]
              "
          ...
          <div class="flex-1 min-w-0 overflow-hidden">
            ...
            <p
              ...
              class="text-xs sm:text-sm text-gray-500 mb-2 leading-snug line-clamp-2"
            >
              {{ camera.description }}
            </p>
            <div class="flex items-center gap-3 text-xs text-gray-400">
              <div class="flex items-center gap-1.5 font-mono">
                ...
                <span class="truncate"
                  ...
                  {{ camera.longitude.toFixed(4) }}</span
                >
              </div>
          ...
          <div
            class="flex-shrink-0 text-gray-400 transition-all group-hover:text-secondary group-hover:translate-x-1"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="20"
      ...
      >
        ...
        <div class="p-6 space-y-6">
          ...
          <div>
            ...
            <div
              class="relative h-[300px] rounded-xl overflow-hidden border-2 border-gray-200"
            >
              <div ref="createMapContainer" class="w-full h-full"></div>
            </div>
            <p
              v-if="newCamera.latitude && newCamera.longitude"
              class="mt-2 text-sm text-green-600"
            >
              📍 Selected: {{ newCamera.latitude.toFixed(6) }},
              {{ newCamera.longitude.toFixed(6) }}
            </p>
          ...
          <div>
            ...
            <input
              ...
              type="text"
              placeholder="e.g., Forest Camera North"
          ...
          <div>
            ...
            <textarea
              ...
              class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-lg focus:outline-none focus:border-blue-500 transition-colors resize-none"
            ></textarea>
          </div>
          ...
          <div class="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                id="camera-lat"
                v-model.number="newCamera.latitude"
                type="number"
                step="0.000001"
            ...
            <div>
              ...
              <input
                id="camera-lng"
                v-model.number="newCamera.longitude"
                type="number"
                step="0.000001"
  ...
  ([lat, lng]) => {
    if (
      ...
      if (createMarker) {
        ...
        createMarker = L.marker([lat as number, lng as number]).addTo(
          createMap,
        );
      }
...
const createCamera = async () => {
  ...
  try {
    ...
    createError.value =
      err instanceof Error ? err.message : "Failed to create camera";
