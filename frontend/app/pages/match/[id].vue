<template>
  <div class="flex-1 bg-gradient-to-b overflow-hidden flex flex-col">
    ...
    <div class="w-full h-[60vh] overflow-hidden mb-2 relative flex-shrink-0">
      ...
      <img
        v-if="showCropped && croppedImageSrc"
        :src="croppedImageSrc"
        alt="Cropped Wildlife Detection"
        class="absolute inset-0 w-full h-full object-contain"
      />

      <!-- Fallback: full image if no bounding box or toggle is off -->
      <img
        v-else-if="mainImageSrc"
        :src="mainImageSrc"
      ...
      <svg
        ...
        :viewBox="`0 0 ${fullImageDimensions.width} ${fullImageDimensions.height}`"
        preserveAspectRatio="xMidYMid meet"
      >
        <rect
          v-for="(detection, index) in imageData.detections"
          :key="index"
      ...
      <!-- Zoom Button Tooltip -->
      <div
        v-if="showZoomTooltip && croppedImageSrc"
        class="absolute top-20 right-4 bg-blue-600 text-white px-4 py-3 rounded-lg shadow-2xl z-50 max-w-xs animate-bounce"
      >
        <div
          class="absolute -top-2 right-8 w-4 h-4 bg-blue-600 transform rotate-45"
        ></div>
        <p class="text-sm font-medium">
          Toggle between cropped and full image view!
        </p>
      </div>

      <div
        class="bg-black/60 text-white px-4 py-2 rounded-lg backdrop-blur-sm absolute top-4 left-4"
      >
      ...
      <div
        v-if="showUnknownTooltip"
        class="absolute top-16 left-[230px] bg-blue-600 text-white px-4 py-3 rounded-lg shadow-2xl z-50 max-w-xs animate-bounce"
      >
        <div
          class="absolute -top-2 left-4 w-4 h-4 bg-blue-600 transform rotate-45"
        ></div>
        <p class="text-sm font-medium">
          Can't identify the animal? Click here to skip this image!
        </p>
      </div>
    </div>
        ...
        >
          <div class="w-full h-full overflow-hidden relative bg-gray-100">
            ...
            >
              ...
              ></div>
              <p class="text-sm font-medium">
                Learn more about this species on Wikipedia!
              </p>
            </div>
            -->
            ...
            <!-- Text Content - Bottom -->
            <div
              class="absolute bottom-0 left-0 right-0 p-6 text-white z-10 pointer-events-none"
            >
              <h3 class="text-3xl font-bold drop-shadow-lg mb-2">
                {{ animal.title }}
              </h3>
              ...
              <div class="flex flex-wrap items-center gap-2 mb-3">
                ...
                >
                  ...
                  {{ getUserDetectionCount(animal.title) }} user{{
                    getUserDetectionCount(animal.title) !== 1 ? "s" : ""
                  }}
                </span>
              </div>

              <!-- <p
                class="text-white/90 text-base leading-relaxed line-clamp-3 drop-shadow-md"
              >
                {{ animal.description }}
              </p> -->
            </div>
...
const mainImageSrc = computed(() => {
  if (imageData.value?.url) {
    const url = imageData.value.url.startsWith("/")
      ? imageData.value.url
      : `/${imageData.value.url}`;
    return `${apiUrl}${url}`;
  }
...
const bboxScaleFactor = computed(() => {
  ...
  const maxDim = Math.max(
    fullImageDimensions.value.width,
    fullImageDimensions.value.height,
  );
...
const createCroppedImage = async () => {
  ...
  if (
    !imageData.value?.detections ||
    imageData.value.detections.length === 0 ||
    !mainImageSrc.value
  ) {
  ...
  const paddedY = Math.max(
    0,
    scaledBbox.y - scaledBbox.height * paddingPercent,
  );
  ...
  ctx.drawImage(
    ...
    paddedY,
    paddedWidth,
    paddedHeight, // Source rectangle
    0,
    0,
    paddedWidth,
    paddedHeight, // Destination rectangle
  );
...
const getUserDetectionCount = (speciesName: string): number => {
  ...
  const detection = userStats.value.user_detections.find(
    (d) => d.name === speciesName,
  );
...
const getAIConfidence = (speciesName: string): number => {
  ...
  const matchingDetections = imageData.value.detections.filter(
    (d) => d.species === speciesName.toLocaleLowerCase(),
  );
...
const fetchImageData = async () => {
  try {
    ...
    const detectedSpecies = [
      ...new Set(data.detections.map((d: Detection) => d.species)),
    ] as string[];
...
const fetchHistoricalSpecies = async (): Promise<string[]> => {
  try {
    const response = await fetchWithAuth(
      `/statistics?period=year&granularity=daily`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
const handleSwipe = () => {
  ...
  if (
    ...
    Math.abs(diffY) < Math.abs(diffX) * 0.5
  ) {
