...
class ModelManager:
    ...
    def load_classification_model(self) -> None:
        ...
        try:
            ...
            if self.region.lower() in ["europe", "european"]:
                ...
                logger.warning(
                    f"Model {self.model_config.classification_model_class} not available, using AI4GAmazonRainforest"
                )
                self.classification_model = pw_classification.AI4GAmazonRainforest()
                self._model_versions["classification"] = "AI4GAmazonRainforest"

            logger.info(
                f"Classification model loaded: {self._model_versions['classification']}"
            )
        except Exception as e:
    ...
    def process_image(
        self, image: np.ndarray, timestamp: Optional[str] = None
    ) -> Tuple[List[AnimalDetection], float]:
        ...
        try:
            ...
            logger.debug(
                f"Running classification pipeline for European enhancement (enabled: {self.enable_european_enhancement})"
            )
            classification_result = self.run_classification_pipeline(
                image, detection_result, timestamp
            )

            # Combine results
            detections = self._combine_results(
                detection_result, classification_result, image, timestamp
            )

            total_time = time.time() - start_time
            logger.debug(
                f"Total processing time: {total_time:.2f}s, found {len(detections)} detections"
            )
            ...
            for i, detection in enumerate(det_boxes):
                ...
                if i < len(classifications):
                    ...
                    classification = classifications[
                        0
                    ]  # Use first classification as fallback
                else:
                ...
                confidence = float(
                    classification.get("confidence", detection.get("confidence", 0.0))
                )
                species = str(
                    classification.get(
                        "class_name", classification.get("species", "unknown")
                    )
                )
                ...
                if classification.get("enhancement_applied", False):
                    ...
                    classification_method = self._model_versions.get(
                ...
                if confidence < 0.2 or species == "unknown":
                    logger.debug(
                        f"Skipping detection with low confidence {confidence} or unknown species {species}"
                    )
                    continue
                ...
                if classification.get("enhancement_applied", False):
                    enhancement_metadata = {
                        "original_classification": classification.get(
    ...
    def _is_generic_result(self, classification_result: Dict[str, Any]) -> bool:
        ...
        species = classification_result.get(
            "prediction",
            classification_result.get(
                "class_name", classification_result.get("species", "")
            ),
        )
        ...
        if self.region == "europe" and self.enable_european_enhancement:
            ...
            if species.lower() in amazon_species:
                logger.debug(
                    f"Amazon species '{species}' will be enhanced to European equivalent"
                )
                return True
    ...
    def run_classification_pipeline(
        ...
        1. Extract detection bounding boxes
        2. Crop image to each detection region
        3. Classify each cropped region using European model (if available)
        4. Fall back to regional model with enhancement
        5. Final fallback to generic classification
        ...
        Returns:
            Classification results dict with "predictions" list (one per detection)
        """
        predictions = []

        try:
            # Extract detection boxes from detection_result
            det_boxes = []
            if detection_result and "detections" in detection_result:
                detections_obj = detection_result["detections"]
                if (
            ...
            for i, detection in enumerate(det_boxes):
                try:
                    ...
                    if (
                        ...
                        logger.debug(
                            f"Skipping detection {i}: cropped region too small"
                        )
                    ...
                    logger.debug(
                        f"Classifying cropped region {i}: {cropped_image.shape}"
                    )

                    # Try European model first (if available)
                    if self.region.lower() in ["europe", "european"]:
                        try:
                            european_result = self._try_european_model(cropped_image)
                            if european_result and not self._is_generic_result(
                                european_result
                            ):
                                logger.debug(
                                    f"Using DeepFaune result for detection {i}"
                                )
                                european_result["enhancement_applied"] = False
                                european_result["classification_method"] = (
                                    "DeepfauneClassifier"
                                )
                                predictions.append(european_result)
                                continue
                        except Exception as e:
                            logger.debug(
                                f"DeepFaune model failed for detection {i}: {e}"
                            )
                    ...
                    try:
                        ...
                        classification_result = self.run_classification(cropped_image)

                        # Enhance if needed
                        if self._is_generic_result(classification_result):
                            enhanced = self.enhance_generic_classification(
                                detection,
            ...
            # Return results in format expected by _combine_results
            if len(predictions) == 1:
                # Single result - return directly (for backward compatibility)
                return predictions[0]
            else:
                # Multiple results - return with "predictions" key
    ...
    def _try_european_model(self, image: np.ndarray) -> Optional[Dict[str, Any]]:
        """Try to use European-specific classification model (DeepFaune).

        Args:
            image: Input image (cropped detection region)
        ...
        try:
            ...
            # Check if DeepFaune model is loaded (v3 or v4)
            classification_version = self._model_versions.get("classification", "")
            if classification_version in [
                "DeepfauneClassifier",
                "DeepfauneClassifier-v4",
            ]:
                result = self.run_classification(image)
                return result
            else:
                logger.debug("DeepFaune model not loaded, using fallback")
                return None
        except Exception as e:
            logger.debug(f"DeepFaune model failed: {e}")
            return None

    def _extract_detection_context(
    ...
    def get_enhancement_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "enhancement_applicable": self.species_mapper.is_enhancement_applicable(
                self.region
            )
            if self.species_mapper
            else False,
        }
