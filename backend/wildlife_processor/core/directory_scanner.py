...
from wildlife_processor.utils.image_utils import is_supported_format, validate_image_file
...
class DirectoryScanner:
    ...
    def scan_directory(self, root_path: Path) -> List[ImageMetadata]:
        ...
        logger.info(f"Found {len(image_metadata_list)} valid images, "
                   f"{len(self.failed_files)} failed, {len(self.skipped_files)} skipped")
        
        return image_metadata_list
    
    def _extract_metadata_from_path(self, file_path: Path, root_path: Path) -> Optional[ImageMetadata]:
    ...
    def _extract_timestamp_from_path(self, file_path: Path, path_parts: tuple) -> datetime:
        ...
        if not self._validate_timestamp(timestamp):
            logger.warning(f"Timestamp seems unreasonable: {timestamp}, using current time")
            timestamp = datetime.now()
    ...
    def _parse_datetime_string(self, datetime_str: str) -> Optional[datetime]:
        ...
        for fmt in formats_to_try:
            try:
                ...
                logger.debug(f"Parsed datetime '{datetime_str}' with format '{fmt}' as {parsed_dt}")
                return parsed_dt
            except ValueError:
                continue
    ...
    def _extract_from_filename(self, filename: str) -> Tuple[Optional[str], Optional[datetime]]:
        ...
        try:
            ...
            if len(parts) >= 4:
                ...
                if len(date_part) == 6 and len(time_part) == 4:
                    ...
                    logger.debug(f"Extracted from filename '{filename}': camera={camera_ref}, time={timestamp}")
