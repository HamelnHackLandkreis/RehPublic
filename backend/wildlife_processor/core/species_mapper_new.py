...
class SpeciesMapper:
    ...
    def map_to_european_species(
        ...
        primary_species = classification.get(
            "prediction",
            classification.get("class_name", classification.get("species", "unknown")),
        )
        ...
        logger.debug(
            f"Analyzing classification: primary='{primary_species}' (conf: {primary_confidence:.3f})"
        )

        # Step 1: Look for direct European species matches in the confidence distribution
        european_species_found = self._find_european_species_in_predictions(
            all_confidences
        )
        if european_species_found:
            species_name, confidence, method = european_species_found
            logger.debug(
                f"Found European species in predictions: {species_name} (conf: {confidence:.3f})"
            )
            return self._create_enhanced_result(
                species_name, confidence, method, primary_species, classification
            )

        # Step 2: Look for Amazon species that can be mapped to European equivalents
        amazon_mapping = self._find_amazon_species_mapping(
            all_confidences, detection, image
        )
        if amazon_mapping:
            species_name, confidence, method = amazon_mapping
            logger.debug(
                f"Mapped Amazon species to European: {species_name} (conf: {confidence:.3f})"
            )
            return self._create_enhanced_result(
                species_name, confidence, method, primary_species, classification
            )
    ...
    def _find_european_species_in_predictions(
        self, all_confidences: List[List]
        ...
        for species_name, confidence in all_confidences:
            ...
            for keyword, european_species in self.european_species_keywords.items():
                if keyword in species_lower:
                    ...
                    logger.debug(
                        f"Found European keyword '{keyword}' in '{species_name}' -> {european_species}"
                    )
    ...
    def _find_amazon_species_mapping(
        self, all_confidences: List[List], detection: Dict[str, Any], image: np.ndarray
    ) -> Optional[Tuple[str, float, str]]:
            ...
            for amazon_species, mapping_info in self.amazon_to_european.items():
                if amazon_species in species_lower:
                    ...
                    if confidence >= mapping_info["threshold"]:
                        european_species = mapping_info.get(
                            size_category, mapping_info["medium"]
                        )

                        # Adjust confidence based on mapping quality
                        if (
                            amazon_species == "procyon"
                        ):  # Raccoon is a good match for fox
                        ...
                        logger.debug(
                            f"Mapped Amazon '{species_name}' -> European '{european_species}' (size: {size_category})"
                        )
                        return (
                            str(european_species),
                            float(adjusted_confidence),
                            f"amazon_mapping_{amazon_species}",
                        )
    ...
    def _create_enhanced_result(
        ...
        habitat_context: Optional[str] = None,
        ...
        logger.debug(
            f"Enhanced classification: {species} (confidence: {confidence:.3f}, method: {method})"
        )
    ...
    def _analyze_animal_size(self, detection: Dict[str, Any], image: np.ndarray) -> str:
        ...
        try:
            ...
            logger.debug(
                f"Bbox area: {bbox_area}, Image area: {image_area}, Relative size: {relative_size:.4f}"
            )
    ...
    def _infer_habitat_from_image(self, image: np.ndarray) -> str:
        ...
        try:
            ...
            if len(image.shape) == 3:
                ...
                green_dominance = avg_colors[1] - np.mean(
                    [avg_colors[0], avg_colors[2]]
                )
    ...
    def _determine_species_by_size(
        self, size_category: str, habitat_context: str, time_context: str
    ) -> Tuple[str, float]:
        ...
        logger.debug(
            f"Selected {selected_species} from {species_list} with confidence multiplier {confidence_multiplier}"
        )
        return selected_species, confidence_multiplier

    def _get_alternative_species_for_result(
        self, selected_species: str, size_category: Optional[str] = None
    ) -> List[str]:
        ...
        if size_category and size_category in self.size_categories:
            ...
            alternatives = [
                species for species in all_species if species != selected_species
            ]
