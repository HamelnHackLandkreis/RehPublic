...
class SpeciesMapper:
    ...
    def map_to_european_species(
        ...
        logger.debug(
            f"Enhanced classification: {probable_species} (confidence: {enhanced_confidence:.3f})"
        )
    ...
    def _determine_species(
        ...
        amazon_mapped_species = self._map_amazon_to_european_species(
            original_species, size_category
        )
        if amazon_mapped_species:
            logger.debug(
                f"Mapped Amazon species '{original_species}' to European species '{amazon_mapped_species}'"
            )
    ...
    def _get_alternative_species(
        self, size_category: str, selected_species: str
    ) -> List[str]:
    ...
    def _map_amazon_to_european_species(
        self, amazon_species: str, size_category: str
    ) -> Optional[str]:
        ...
        if species_lower in amazon_to_european_mapping:
            ...
            return size_mapping.get(
                size_category, list(size_mapping.values())[0]
