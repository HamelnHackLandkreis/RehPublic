...
REGIONAL_MODELS = {
    ...
    "europe": ModelConfig(
        region="europe",
        classification_model_class="DeepfauneClassifier",
        model_name="DeepFaune European Wildlife Classifier",
...
def get_model_config(region: str) -> ModelConfig:
    ...
    if region not in REGIONAL_MODELS:
        raise ValueError(
            f"Unknown region: {region}. Available regions: {list(REGIONAL_MODELS.keys())}"
        )
