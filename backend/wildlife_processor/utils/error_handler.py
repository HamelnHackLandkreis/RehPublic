...
class ErrorHandler:
    ...
    def handle_image_error(self, image_path: Path, error: Exception) -> None:
        ...
        if (
            "corrupted" in str(error).lower()
            or "cannot identify image file" in str(error).lower()
        ):
            logger.warning(f"Image file appears to be corrupted: {image_path}")
            logger.info(
                "Suggestion: Check if the image file is valid and not truncated"
            )
            ...
            logger.info(
                "Suggestion: Try processing smaller batches or reduce image resolution"
            )
    ...
    def handle_output_error(self, output_path: Path, error: Exception) -> None:
        ...
        if "permission" in error_str:
            ...
            logger.info(
                "  2. Try writing to a different location (e.g., your home directory)"
            )
            ...
            logger.info(
                "Suggestion: The output directory will be created automatically"
            )
        else:
            logger.info("Suggestions:")
