            ...
            for detection in detections[:2]:  # Show first 2 detections per camera
                for animal in detection.detections:
                    ...
                    if animal.enhancement_metadata:
                        original = animal.enhancement_metadata.get('original_classification', 'N/A')
                        enhancement_info = f" (enhanced from '{original}')"
                    
                    alternatives = ""
                    if animal.alternative_species:
                        alternatives = f", alternatives: {', '.join(animal.alternative_species)}"
                    
                    print(f"  - {animal.species} (confidence: {animal.confidence:.3f}){enhancement_info}{alternatives}")
