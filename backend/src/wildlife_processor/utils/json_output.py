            ...
            for detection_result in detection_results:
                ...
                for detection in detection_result.detections:
                    ...
                    if detection.alternative_species:
                        detection_dict["alternative_species"] = (
                            detection.alternative_species
                        )

                    if detection.enhancement_metadata:
                        detection_dict["enhancement_metadata"] = (
                            detection.enhancement_metadata
                        )
        ...
        if results_by_camera:
            ...
            for camera_ref, camera_results in results_by_camera.items():
                camera_detections = sum(
                    len(result["detections"]) for result in camera_results
                )
                report_lines.append(
                    f"  {camera_ref}: {len(camera_results)} images, {camera_detections} detections"
                )
